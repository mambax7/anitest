...
like '$fid%' and roft='0'");*/
$query = $GLOBALS['xoopsDB']->queryF('SELECT ID,NAAM,user2,user3,user4,id_eigenaar,id_fokker,user 
FROM ' . $GLOBALS['xoopsDB']->prefix('stamboom') . " 
WHERE NAAM LIKE '%$fid%' AND roft='0' ORDER BY NAAM ASC limit 0, 50");
...
<div id="vader" style="width:500px;">
    ...
    if ($cnt != 0) {
        ...
        <table name="father" class="father" style="overflow:scroll;">
            ...
            while ($row = $GLOBALS['xoopsDB']->fetchBoth($query)) {
                $id   = $row['ID'];
                $name = $row['NAAM']; ?>
