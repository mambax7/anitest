...
$queryfoks = $GLOBALS['xoopsDB']->queryF('SELECT ID, NAAM, id_eigenaar from ' . $xoopsDB->prefix('stamboom') . " where id_eigenaar= '$fid' ORDER BY NAAM  asc");
...
if ($coont != 0) {
    ...
    <table id="tabsdsatyle">
        ...
        <?php while ($row = $GLOBALS['xoopsDB']->fetchBoth($queryfoks)) {
            ...
            <tr>
                ...
                <?php
                $sqlown    = $GLOBALS['xoopsDB']->queryF('select firstname,lastname from ' . $xoopsDB->prefix('eigenaar') . " where ID='$row[id_eigenaar]'");
    ...
    position: absolute; right: 311px; " onclick="update();">Update owner</a></div>
    <?php
