...
<div id="containers">
    ...
    <fieldset id="checkaArray">
        ...
        <form id="testtable" method="post" action="nbscript.php?a=<?php echo $_REQUEST['a']; ?>">
            <table id="tabssatyle" style="width:100%; border: 1px solid black;">
                ...
                <?php
                $trunk = '';
                $trunk = $GLOBALS['xoopsDB']->queryF('TRUNCATE TABLE tempdatas');
                //echo "SELECT * FROM `mSVsD_eigenaar` where firstname like '%$_REQUEST[a]%' order by ID";
                $conts    = '';
                $fnlsname = '';
                $data     = '';
                $fnlsname = $GLOBALS['xoopsDB']->queryF('SELECT * FROM `' . $xoopsDB->prefix('eigenaar') . "` where firstname like '%$_REQUEST[a]%' ");
                ...
                if ($conts != 0) {
                    ...
                    while ($data = $GLOBALS['xoopsDB']->fetchBoth($fnlsname)) {
                        ...
                        $bredder = $GLOBALS['xoopsDB']->queryF('select * from ' . $xoopsDB->prefix('stamboom') . " where id_fokker=$data[ID]");
                        ...
                        if ($bcnt != 0) {
                            ...
                            $inss = '';
                            $inss = $GLOBALS['xoopsDB']->queryF('INSERT INTO ' . $xoopsDB->prefix('tempdatas') . " (ins) VALUES ('.$data[ID].')"); ?>
                            ...
                            while ($maildata = $GLOBALS['xoopsDB']->fetchBoth($sqlmail)) {
                                $quer   = '';
                                $qqquer = '';
                                $quer   = $GLOBALS['xoopsDB']->queryF('select count(*) as cntss from ' . $xoopsDB->prefix('stamboom') . " where id_fokker=$maildata[ID]");
                                $qqquer = $GLOBALS['xoopsDB']->fetchBoth($quer);
                                if ($qqquer[cntss] != 0) {
                                    if ($maildata[ID] != $data['ID']) {
                                        $inss = '';
                                        $inss = $GLOBALS['xoopsDB']->queryF('INSERT INTO ' . $xoopsDB->prefix('tempdatas') . " (ins) VALUES ('$maildata[ID]')"); ?>
                            ...
                            if ($data[lastname] != '') {
                                ...
                                $fnlssname = '';
                                $connts    = '';
                                $fnlssname = $GLOBALS['xoopsDB']->queryF('SELECT * FROM `' . $xoopsDB->prefix('eigenaar') . "` where lastname like '%$data[lastname]%' ");
                                $connts    = $GLOBALS['xoopsDB']->getRowsNum($fnlssname);
                                if ($connts != 0) {
                                    $fnlssnamedataa = '';
                                    while ($fnlssnamedataa = $GLOBALS['xoopsDB']->fetchBoth($fnlssname)) {
                                        $bredders = $GLOBALS['xoopsDB']->queryF('select count(*) as bcnts from ' . $xoopsDB->prefix('stamboom') . " where id_fokker=$fnlssnamedataa[ID]");
                                        ...
                                        if ($bcnts[bcnts] != 0) {
                                            if ($fnlssnamedataa[ID] != $data['ID']) {
                                                ...
                                                <?php $inass = '';
                                                $inass       = $GLOBALS['xoopsDB']->queryF('INSERT INTO ' . $xoopsDB->prefix('tempdatas') . " (ins) VALUES ('.$fnlssnamedataa[ID].')"); ?>
                                                ...
                                                while ($maildata = $GLOBALS['xoopsDB']->fetchBoth($sqlmail)) {
                                                    $quer   = '';
                                                    $qqquer = '';
                                                    //echo "select count(*) as cntss from mSVsD_stamboom where id_fokker=$maildata[ID]";
                                                    $quer   = $GLOBALS['xoopsDB']->queryF('select count(*) as cntss from ' . $xoopsDB->prefix('stamboom') . " where id_fokker=$maildata[ID]");
                                                    $qqquer = $GLOBALS['xoopsDB']->fetchBoth($quer);
                                                    if ($qqquer[cntss] != 0) {
                ...
                $lsname = '';
                $concts = '';
                $lsname = $GLOBALS['xoopsDB']->queryF('SELECT * FROM `' . $xoopsDB->prefix('eigenaar') . "` where lastname like '%$_REQUEST[a]%' ");
                ...
                if ($concts != 0) {
                    ...
                    while ($daaata = $GLOBALS['xoopsDB']->fetchBoth($lsname)) {
                        ...
                        $lbredders = $GLOBALS['xoopsDB']->queryF('select count(*) as blcnts from ' . $xoopsDB->prefix('stamboom') . " where id_fokker=$daaata[ID]");
                        ...
                        if ($lbcnts[blcnts] != 0) {
                            ...
                            <?php
                            $inss = '';
                            $inss = $GLOBALS['xoopsDB']->queryF('INSERT INTO ' . $xoopsDB->prefix('tempdatas') . " (ins) VALUES ('.$daaata[ID].')");
                            //echo "select uname,email from mSVsD_users where uid='$data[user]'";
                            ...
                            if ($daaata[firstname] != '') {
                                ...
                                $fnlsssnamedata = '';
                                $connts         = '';
                                $fnlssname      = $GLOBALS['xoopsDB']->queryF('SELECT * FROM `' . $xoopsDB->prefix('eigenaar') . "` where firstname like '%$daaata[firstname]%' ");
                                $connts         = $GLOBALS['xoopsDB']->getRowsNum($fnlssname);
                                if ($connts != 0) {
                                    while ($fnlsssnamedata = $GLOBALS['xoopsDB']->fetchBoth($fnlssname)) {
                                        $lnbredders = '';
                                        $lnbcnts    = '';
                                        $lnbredders = $GLOBALS['xoopsDB']->queryF('select count(*) as blncnts from ' . $xoopsDB->prefix('stamboom') . " where id_fokker=$daaata[ID]");
                                        ...
                                        if ($lnbcnts[blncnts] != 0) {
                                            if ($fnlsssnamedata[ID] != $daaata['ID']) {
                                                ...
                                                <?php
                                                $inssd = '';
                                                $inssd = $GLOBALS['xoopsDB']->queryF('INSERT INTO ' . $xoopsDB->prefix('tempdatas') . " (ins) VALUES ('.$fnlsssnamedata[ID].')");
                                                //echo "select uname,email from mSVsD_users where uid='$data[user]'";
                                                $ussaname = '';
                                                $udsaata  = '';
                                            ...
                                            while ($maildata = $GLOBALS['xoopsDB']->fetchBoth($sqlmail)) {
                                                ...
                                                if ($qqquer[cntss] != 0) {
                                                    if ($maildata[ID] != $data['ID']) {
                                                        $inssd = '';
                                                        $inssd = $GLOBALS['xoopsDB']->queryF('INSERT INTO ' . $xoopsDB->prefix('tempdatas') . " (ins) VALUES ('.$maildata[ID].')"); ?>
                ...
                $conts     = '';
                $data      = '';
                $fnlsnames = $GLOBALS['xoopsDB']->queryF('SELECT * FROM `' . $xoopsDB->prefix('eigenaar') . "` where firstname like '%$_REQUEST[a]%' order by ID");
				...
				<input type="hidden" name="idvalues" value="<?php echo $datas['ID']; ?>"/>-->
                            ...
                            //echo "select uname,email from mSVsD_users where uid='$data[user]'";
                            $inss   = '';
                            $inss   = $GLOBALS['xoopsDB']->queryF('INSERT INTO ' . $xoopsDB->prefix('tempdatas') . " (ins) VALUES ('.$datas[ID].')");
				...
				<td align="center"><?php //echo $udata['email'];?></td>-->
                            ...
                            if ($datas[lastname] != '') {
                                ...
                                $fnlssname = '';
                                $connts    = '';
                                $fnlssname = $GLOBALS['xoopsDB']->queryF('SELECT * FROM `' . $xoopsDB->prefix('eigenaar') . "` where lastname ='$datas[lastname]' order by ID");
                                $connts    = $GLOBALS['xoopsDB']->getRowsNum($fnlssname);
                                if ($connts != 0) {
                                    $fnlssnamedata = '';
                                    while ($fnlssnamedata = $GLOBALS['xoopsDB']->fetchBoth($fnlssname)) {
                                        ...
                                        if ($bcnts[bcnts] == 0) {
                                            if ($fnlssnamedata[ID] != $datas['ID']) {
                                                ...
                                                <?php
                                                $inass = '';
                                                $inass = $GLOBALS['xoopsDB']->queryF('INSERT INTO ' . $xoopsDB->prefix('tempdatas') . " (ins) VALUES ('.$fnlssnamedata[ID].')"); ?>
                                                <?php
                ...
                if ($concts != 0) {
                    ...
                    while ($daasata = $GLOBALS['xoopsDB']->fetchBoth($lsname)) {
                        ...
                        if ($lbcnts[blcnts] == 0) {
                            ...
                            <?php
                            $insds = '';
                            $insds = $GLOBALS['xoopsDB']->queryF('INSERT INTO ' . $xoopsDB->prefix('tempdatas') . " (ins) VALUES ('.$daasata[ID].')");
                            if ($daasata[firstname] != '') {
                                ...
                                $lnbcnts         = '';
                                $connts          = '';
                                $fnlssname       = $GLOBALS['xoopsDB']->queryF('SELECT * FROM `' . $xoopsDB->prefix('eigenaar') . "` where firstname ='$daasata[firstname]' ");
                                $connts          = $GLOBALS['xoopsDB']->getRowsNum($fnlssname);
                                if ($connts != 0) {
                                    $lnbredders = '';
                                    $lnbcnts    = '';
                                    while ($fnlsssnamedatas = $GLOBALS['xoopsDB']->fetchBoth($fnlssname)) {
                                        ...
                                        if ($lnbcnts[blncnts] == 0) {
                                            if ($fnlsssnamedatas[ID] != $daasata['ID']) {
                                                ...
                                                $inssd = '';
                                                $inssd = $GLOBALS['xoopsDB']->queryF('INSERT INTO ' . $xoopsDB->prefix('tempdatas') . " (ins) VALUES ('.$fnlsssnamedatas[ID].')");
                                            } ?>
                ...
                $btempdata = $GLOBALS['xoopsDB']->queryF('SELECT DISTINCT ins FROM ' . $xoopsDB->prefix('tempdatas'));
                $j         = 0;
                while ($fetch = $GLOBALS['xoopsDB']->fetchBoth($btempdata)) {
                    ...
                    while ($breddata = $GLOBALS['xoopsDB']->fetchBoth($bdata)) {
                        ...
                        $breddercount = '';
                        $bcntcount    = '';
                        $breddercount = $GLOBALS['xoopsDB']->queryF('select count(*) as bcnt from ' . $xoopsDB->prefix('stamboom') . " where id_fokker=$breddata[ID]");
        ...
        <form id="testtaable" method="post" action="">
            <fieldset>
                    ...
                    foreach ($_POST['checks'] as $chek) { //echo $a;//print_r($chek);
                        ...
                        if ($cont != 0) {
                            ...
                            while ($datas = $GLOBALS['xoopsDB']->fetchBoth($sql)) {
                                ...
                                <tr class="<?php echo $even; ?>">
                                    <td align="center"><input class='checksas' id="chees<?php echo $b; ?>" type="checkbox" value="<?php echo $datas['ID']; ?>" name="chees[]"/></td>
                ...
                <table id="tabsdsatyle" style="width:100%;">
                    ...
                    $sqlquername = $GLOBALS['xoopsDB']->queryF('SELECT DISTINCT tempdata.id_eig ,  tempdatas.ins FROM ' . $xoopsDB->prefix('tempdata') . ' 
			 RIGHT JOIN ' . $xoopsDB->prefix('tempdatas') . ' ON tempdata.id_eig = tempdatas.ins');
    ...
    <fieldset id="checkemptyd" style="display:none;">
        ...
        <table id="tabssasatyle" style="width:100%;">
            <thead style="background-color:lightgrey; border:1px solid white; font-family: Verdana,Arial,Helvetica,sans-serif; font-size: 62.5%;">
            <th>Breeder ID</th>
            <th>Firstname</th>
            <th>Lastname</th>
            ...
            //echo $_REQUEST['a'];
            //echo "select firstname,lastname from mSVsD_eigenaar  where ID = '".$_REQUEST['a']."'";
            $fnlname = $GLOBALS['xoopsDB']->queryF('SELECT *  FROM ' . $xoopsDB->prefix('tempdatas'));
            ...
            while ($datad = $GLOBALS['xoopsDB']->fetchBoth($fnlname)) {
                ...
                $queer = $GLOBALS['xoopsDB']->queryF('select count(*) as counts from ' . $xoopsDB->prefix('stamboom') . " where id_fokker='$data[ID]'");
    ...
    <fieldset id="checkaArray">
        <legend style="background-color:#666666; width:100%; color:white; font-family: Verdana,Arial,Helvetica,sans-serif;"
        =>Breeder/Owner with empty joeys</legend>
        <form id="testtablse" method="post" action="">
            <table id="tabssaatyle" style="width:100%;">
                <thead style="background-color:lightgrey; border:1px solid white; font-family: Verdana,Arial,Helvetica,sans-serif; font-size: 62.5%;">
                            ...
                            if ($datas[lastname] != '') {
                                ...
                                if ($connts != 0) {
                                    ...
                                    while ($fnlssnamedata = $GLOBALS['xoopsDB']->fetchBoth($fnlssname)) {
                                        ...
                                        $bcnts    = $GLOBALS['xoopsDB']->fetchBoth($bredders);
                                        if ($bcnts[bcnts] == 0) {
                                            if ($fnlssnamedata[ID] != $datas['ID'] && $datas[firstname] != $fnlssnamedata[firstname]) {
                                                $arr[] = $fnlssnamedata[ID]; ?>
                                                </tr>
                                            <tr class="<?php echo $even; ?>">
                                                ...
                                                //$inass=$GLOBALS['xoopsDB']->queryF("INSERT INTO " . $xoopsDB->prefix('tempdatas') . " (ins) VALUES ('.$fnlssnamedata[ID].')");
                                                //echo "select uname,email from mSVsD_users where uid='$data[user]'";
                                                $uussaname = $GLOBALS['xoopsDB']->queryF('select uname,email FROM ' . $xoopsDB->prefix('users') . " WHERE uid='$fnlssnamedata[user]'");
                                                $uudsaata  = $GLOBALS['xoopsDB']->fetchBoth($uussaname); ?>
                                                <td align="center"><?php echo $uudsaata['uname']; ?></td>
                                                <td align="center"><?php echo $uudsaata['email']; ?></td>
                                                </tr><?php
                ...
                if ($concts != 0) {
                    ...
                    while ($daasata = $GLOBALS['xoopsDB']->fetchBoth($lsname)) {
                        ...
                        <tr class="<?php echo $even; ?>">
                            ...
                            <td align="center"><?php echo $daasata['ID']; ?></td>
                            <td align="center"><?php echo $daasata['firstname']; ?></td>
                            <td align="center"><?php echo $daasata['lastname']; ?></td>
                            <td align="center"><?php echo $daasata['emailadres']; ?></td>
                            <input type="hidden" name="values" value="<?php echo $daasata['lastname'] . ',' . $daasata['firstname']; ?>"/>
                            <input type="hidden" name="idvalues" value="<?php echo $daasata['ID']; ?>"/>
                            <?php
                            //echo "select uname,email from mSVsD_users where uid='$data[user]'";
                            $usaaname  = '';
                            $udaata    = '';
                            $usaaaname = $GLOBALS['xoopsDB']->queryF('select uname,email FROM ' . $xoopsDB->prefix('users') . " WHERE uid='$daasata[user]'");
                            $udaaata   = $GLOBALS['xoopsDB']->fetchBoth($usaaaname); ?>
                            <td align="center"><?php echo $udaaata['uname']; ?></td>
                            <td align="center"><?php echo $udaaata['email']; ?></td>

                            <?php
                            //$insds=$GLOBALS['xoopsDB']->queryF("INSERT INTO " . $xoopsDB->prefix('tempdatas') . " (ins) VALUES ('.$daaata[ID].')");
                            if ($daaata[firstname] != '') {
                                //echo "SELECT * FROM `mSVsD_eigenaar` where firstname ='$data[firstname]'";
                                $fnlssname       = '';
                                $fnlsssnamedatas = '';
                                $fnlssname       = $GLOBALS['xoopsDB']->queryF('SELECT * FROM `' . $xoopsDB->prefix('eigenaar') . "` where firstname ='$daaata[firstname]' ");
                                $connts          = $GLOBALS['xoopsDB']->getRowsNum($fnlssname);
                                if ($connts != 0) {
                                    while ($fnlsssnamedatas = $GLOBALS['xoopsDB']->fetchBoth($fnlssname)) {
                                        $lnbredders = $GLOBALS['xoopsDB']->queryF('select count(*) as blncnts from ' . $xoopsDB->prefix('stamboom') . " where id_eigenaar=$fnlsssnamedatas[ID] or  id_fokker=$fnlsssnamedatas[ID]");
                                        $lnbcnts    = $GLOBALS['xoopsDB']->fetchBoth($lnbredders);
                                        if ($lnbcnts[blncnts] == 0) {
                                            if ($fnlsssnamedatas[ID] != $daaata['ID'] && $daaata[firstname] != $fnlsssnamedatas[firstname]) {
                                                $arr[] = $fnlsssnamedatas[ID]; ?>
                                                </tr>

                                            <tr class="<?php echo $even; ?>">

                                                <td align="center"><?php echo $fnlsssnamedatas['ID']; ?></td>
                                                <td align="center"><?php echo $fnlsssnamedatas['firstname']; ?></td>
                                                <td align="center"><?php echo $fnlsssnamedatas['lastname']; ?></td>
                                                <td align="center"><?php echo $fnlsssnamedatas['emailadres']; ?></td>
                                                <input type="hidden" name="values" value="<?php echo $fnlsssnamedatas['lastname'] . ',' . $fnlsssnamedatas['firstname']; ?>"/>
                                                <input type="hidden" name="idvalues" value="<?php echo $fnlsssnamedatas['ID']; ?>"/>
                                                <?php
                                                $inssd = $GLOBALS['xoopsDB']->queryF('INSERT INTO ' . $xoopsDB->prefix('tempdatas') . " (ins) VALUES ('.$fnlsssnamedatas[ID].')");
                                                //echo "select uname,email from mSVsD_users where uid='$data[user]'";
                                                $usssaname = $GLOBALS['xoopsDB']->queryF('select uname,email FROM ' . $xoopsDB->prefix('users') . " WHERE uid='$fnlsssnamedatas[user]'");
                                                $udsasata  = $GLOBALS['xoopsDB']->fetchBoth($usssaname); ?>
                                                <td align="center"><?php echo $udsasata['uname']; ?></td>
                                                <td align="center"><?php echo $udsasata['email']; ?></td>
                                                </tr><?php
                ...
                <!-- <tr id="even"><td style="background-color:white;"> <input type="submit" name="submts"  value="select gliders"/></td></tr>-->
                </tbody>
            </table>
        </form>
    </fieldset>
    ...
    <fieldset>
        <form name="back" action="http://thepetglider.com/pedigree/modules/animal/o_b_update/mainpage.php" method="post">

            <table>
                <tr>
                    <td><input type="submit" name="sumu" value="Go Back"/></td>
                </tr>
            </table>
        </form>
    </fieldset>
...
<div id="popup_boxs" style="display:none;">    <!-- OUR PopupBox DIV-->
    <h3>Joeys Associated with Owner</h3>
    <div id="id_owns"></div>
    <div><a style="color: #6FA5E2; font-size: 20px; font-weight: 500; line-height: 15px;
    position: static;  right: 5px;" onclick="oked();">Delete</a></div>
