...
<select name="state_select" id="id_eigenaar" style="width:250px;">

    <?php
    while ($row = $GLOBALS['xoopsDB']->fetchBoth($queryfoks)) {
        $queryfokid = $GLOBALS['xoopsDB']->queryF('SELECT count(*) as cnts from ' . $xoopsDB->prefix('stamboom') . " where id_eigenaar='$row[ID]' ");
