        ...
        <form id="testtable" method="post" action="">
            <table id="tabssatyle" style="width:100%;">
                ...
                while ($data = $GLOBALS['xoopsDB']->fetchBoth($getres)) {
                    ...
                    $bredd = $GLOBALS['xoopsDB']->queryF('select * from ' . $xoopsDB->prefix('stamboom') . " where id_eigenaar!='$data[ID]'");
                    //while($bredata=$GLOBALS['xoopsDB']->fetchBoth($bredd)){
                    $brecont = $GLOBALS['xoopsDB']->getRowsNum($bredd);
                    if ($brecont != 0) {
                        if ($i % 2 == 0) {
                            $even = 'even';
                        } else {
            ...
            <fieldset>
                ...
                <table id="tabsdsatyle" style="width:100%;">
                    ...
                    while ($sqlquernameres = $GLOBALS['xoopsDB']->fetchBoth($sqlquer)) {
                        $obredd = '';
                        $obredd = $GLOBALS['xoopsDB']->queryF('select * from ' . $xoopsDB->prefix('stamboom') . " where id_fokker='$sqlquernameres[ID]'");
                        //while($bredata=$GLOBALS['xoopsDB']->fetchBoth($bredd)){
                        $obrecont = 0;
