...
function sire()
{
    ...
    if (!isset($_GET['r'])) {
        ...
        //if ($user==674 || $user== 2 || $user== 20)
        //{
        //$userfileds=$_POST['user'.$fields[$i]];
        //echo 'filed data:'.$userfileds;
        //echo 'Data:'.$usersql;
        //echo 'Gender'.$row['roft'];
        echo 'kanna code:';
        ...
        $random     = $finalvalue;
        //$userid=$user;
        //echo 'The Proc code:'.$proc;
        echo $random;
        //}
        //else
        //{
        //insert into stamboom_temp
        //$query = "INSERT INTO ".$GLOBALS['xoopsDB']->prefix("stamboom_temp")." VALUES ('".$random."','".unhtmlentities($name)."','".$id_eigenaar."','".$id_fokker."','".$user."','".$roft."','','','".$foto."', ''".$usersql.")";
        //echo $query; die();
    ...
    $pages .= '-&nbsp;';
    $pages .= '<a href="add_dog.php?f=sire&r=1&random=' . $random . '&l=�">�</a>&nbsp;';
    $pages .= '<a href="add_dog.php?f=sire&r=1&random=' . $random . '&l=�">�</a>&nbsp;';
    //create linebreak
    $pages .= '<br>';
...
function dam()
{
    ...
    if (!isset($_GET['r'])) {
        ...
        //insert into stamboom_temp
        echo 'kanna code';
        //ho 'Mother'.$_GET['selsire'];

        //ho 'final value'.$random;
    ...
    $pages .= '-&nbsp;';
    $pages .= '<a href="add_dog.php?f=dam&r=1&random=' . $random . '&l=�">�</a>&nbsp;';
    $pages .= '<a href="add_dog.php?f=dam&r=1&random=' . $random . '&l=�">�</a>&nbsp;';
    $pages .= '<br>';
    //create previous button
    if ($numpages > 1) {
...
function check()
{
    ...
    while ($row = $GLOBALS['xoopsDB']->fetchArray($result)) {
        ...
        //echo 'Data:'.$usersql;
        //echo 'Single:'.$row['user'.$fields[0]];
        //echo 'Gender'.$row['roft'];
        //echo 'kanna code:';
        //$proc=$GLOBALS['xoopsDB']->queryF("CALL UPDATE_MOTHER ('".$row['NAAM']."','".$row['id_eigenaar']."','".$row['id_fokker']."','".$row['user']."','".$row['roft']."','".$_GET['seldam']."','".$row['vader']."','".addslashes($row['foto'])."','','".$row['user'.$fields[0]]."','".$row['user'.$fields[1]]."','".$row['user'.$fields[2]]."','".$row['user'.$fields[3]]."','".$row['user'.$fields[4]]."')")or die($GLOBALS['xoopsDB']->error());
        //mysqli_result($proc);
        //echo 'The Proc code:'.$proc;

        //echo 'final value'.$random;
        //$umproc=$GLOBALS['xoopsDB']->queryF("CALL UPDATE_MOTHER(".$_GET['seldam'].",".$random.")")or die($GLOBALS['xoopsDB']->error());
        //mysqli_result($umproc);
        //echo 'pet code';

        //$query = "INSERT INTO ".$GLOBALS['xoopsDB']->prefix("stamboom")." VALUES ('','".$row['NAAM']."','".$row['id_eigenaar']."','".$row['id_fokker']."','".$row['user']."','".$row['roft']."','".$_GET['seldam']."','".$row['vader']."','".addslashes($row['foto'])."',''".$usersql.")";
        //$GLOBALS['xoopsDB']->queryF($query);
        //echo $query; die();
        //}
    }
