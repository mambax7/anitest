...
function set_name($ID)
{
    ...
    if ($ID) {
        ...
        $name        = $ani[1];
        if ($sql2bis) {  // true for E.R.o'S. only $name = html_accents($name) ;
            //$affx = $ani[5] ;  // affix-ID
            if ($affx) {
...
$xoopsTpl->assign('SSDexplain', strtr(_PED_COI_SSDEX, array('[father]' => $moduleConfig['father'], '[mother]' => $moduleConfig['mother'], '[animalType]' => $moduleConfig['animalTpyes'])));
