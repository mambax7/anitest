...
$result = $GLOBALS['xoopsDB']->query('SELECT count(*) FROM ' . $GLOBALS['xoopsDB']->prefix('stamboom') . " WHERE roft='0'");
list($countmales) = $GLOBALS['xoopsDB']->fetchRow($result);

//query to count female dogs
$result = $GLOBALS['xoopsDB']->query('SELECT count(*) FROM ' . $GLOBALS['xoopsDB']->prefix('stamboom') . " WHERE roft='1'");
...
for ($i = 0, $iMax = count($fields); $i < $iMax; $i++) {
    ...
    if ($userfield->active() && $userfield->inadvanced()) {
        $queryString = 'select count(p.user'
                       . $fields[$i]
                       . ') as X, p.user'
                       . $fields[$i]
                       . ' as p_user'
                       . $fields[$i]
                       . ', b.ID as b_id, b.value as b_value from '
                       . $GLOBALS['xoopsDB']->prefix('stamboom')
                       . ' p LEFT JOIN '
                       . $GLOBALS['xoopsDB']->prefix('stamboom_lookup' . $fields[$i])
                       . ' b ON p.user'
                       . $fields[$i]
                       . ' = b.ID GROUP BY p.user'
                       . $fields[$i]
                       . ' ORDER BY X DESC';
        ...
        while ($row = $GLOBALS['xoopsDB']->fetchArray($result)) {
            ...
            $data[$piecount][2] = '#' . hexdec(mt_rand(1, 255)) . hexdec(mt_rand(1, 255)) . hexdec(mt_rand(1, 255));
