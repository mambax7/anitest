...
function pedigree_main($ID)
{
	...
	LEFT JOIN ' . $GLOBALS['xoopsDB']->prefix('stamboom') . ' f ON d.vader = f.id 
	LEFT JOIN ' . $GLOBALS['xoopsDB']->prefix('stamboom') . ' m ON d.moeder = m.id 
	LEFT JOIN ' . $GLOBALS['xoopsDB']->prefix('stamboom') . ' ff ON f.vader = ff.id 
	LEFT JOIN ' . $GLOBALS['xoopsDB']->prefix('stamboom') . ' fff ON ff.vader = fff.id 
	LEFT JOIN ' . $GLOBALS['xoopsDB']->prefix('stamboom') . ' ffm ON ff.moeder = ffm.id 
	LEFT JOIN ' . $GLOBALS['xoopsDB']->prefix('stamboom') . ' mf ON m.vader = mf.id 
	LEFT JOIN ' . $GLOBALS['xoopsDB']->prefix('stamboom') . ' mff ON mf.vader = mff.id 
	LEFT JOIN ' . $GLOBALS['xoopsDB']->prefix('stamboom') . ' mfm ON mf.moeder = mfm.id 
	LEFT JOIN ' . $GLOBALS['xoopsDB']->prefix('stamboom') . ' fm ON f.moeder = fm.id 
	LEFT JOIN ' . $GLOBALS['xoopsDB']->prefix('stamboom') . ' fmf ON fm.vader = fmf.id 
	LEFT JOIN ' . $GLOBALS['xoopsDB']->prefix('stamboom') . ' fmm ON fm.moeder = fmm.id 
	LEFT JOIN ' . $GLOBALS['xoopsDB']->prefix('stamboom') . ' mm ON m.moeder = mm.id 
	LEFT JOIN ' . $GLOBALS['xoopsDB']->prefix('stamboom') . ' mmf ON mm.vader = mmf.id 
	LEFT JOIN ' . $GLOBALS['xoopsDB']->prefix('stamboom') . ' mmm ON mm.moeder = mmm.id 
	LEFT JOIN ' . $GLOBALS['xoopsDB']->prefix('stamboom') . ' ffff ON fff.vader = ffff.id
	LEFT JOIN ' . $GLOBALS['xoopsDB']->prefix('stamboom') . ' ffmf ON ffm.vader = ffmf.id
	LEFT JOIN ' . $GLOBALS['xoopsDB']->prefix('stamboom') . ' fmff ON fmf.vader = fmff.id
	LEFT JOIN ' . $GLOBALS['xoopsDB']->prefix('stamboom') . ' fmmf ON fmm.vader = fmmf.id
	LEFT JOIN ' . $GLOBALS['xoopsDB']->prefix('stamboom') . ' mmff ON mmf.vader = mmff.id
	LEFT JOIN ' . $GLOBALS['xoopsDB']->prefix('stamboom') . ' mfff ON mff.vader = mfff.id
	LEFT JOIN ' . $GLOBALS['xoopsDB']->prefix('stamboom') . ' mfmf ON mfm.vader = mfmf.id
	LEFT JOIN ' . $GLOBALS['xoopsDB']->prefix('stamboom') . ' mmmf ON mmm.vader = mmmf.id
	LEFT JOIN ' . $GLOBALS['xoopsDB']->prefix('stamboom') . ' fffm ON fff.moeder = fffm.id
	LEFT JOIN ' . $GLOBALS['xoopsDB']->prefix('stamboom') . ' ffmm ON ffm.moeder = ffmm.id
	LEFT JOIN ' . $GLOBALS['xoopsDB']->prefix('stamboom') . ' fmfm ON fmf.moeder = fmfm.id
	LEFT JOIN ' . $GLOBALS['xoopsDB']->prefix('stamboom') . ' fmmm ON fmm.moeder = fmmm.id
	LEFT JOIN ' . $GLOBALS['xoopsDB']->prefix('stamboom') . ' mmfm ON mmf.moeder = mmfm.id
	LEFT JOIN ' . $GLOBALS['xoopsDB']->prefix('stamboom') . ' mffm ON mff.moeder = mffm.id
	LEFT JOIN ' . $GLOBALS['xoopsDB']->prefix('stamboom') . ' mfmm ON mfm.moeder = mfmm.id
	LEFT JOIN ' . $GLOBALS['xoopsDB']->prefix('stamboom') . " mmmm ON mmm.moeder = mmmm.id
	where d.id=$ID";
