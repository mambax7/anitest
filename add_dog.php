...
//document.getElementById('photos').focus();
    ...
    $(document).ready(function () {
        ...
        function ajaxFunction() {                      //-----------------------------------------------sagar code
...
//    $GLOBALS['xoopsLogger']->activated = false;
    ...
    if ($numresults >= 1 && !isset($_GET['r'])) {
        ...
        $form->addElement(new XoopsFormHidden('random', $random));
        //        $form->addElement(new XoopsFormHidden('NAAM', htmlspecialchars($_POST['NAAM'], ENT_QUOTES)));
        $form->addElement(new XoopsFormHidden('NAAM', htmlspecialchars(Request::getString('NAAM', '', 'POST'), ENT_QUOTES)));
        //find userid from previous form
        $form->addElement(new XoopsFormHidden('user', Request::getString('user', '', 'POST')));
        //$form->addElement(new XoopsFormHidden('xoops_upload_file[]', 'photo'));

        //rajesh code start
        $form->addElement(new XoopsFormLabel('<b>' . _PED_FLD_NAME . '</b>', stripslashes(stripslashes(Request::getString('NAAM', '', 'POST')))));
        //gender
